...
export const AttendanceView: React.FC = () => {
  ...
  const handleTogglePunch = async () => {
    ...
    try {
      if (!isCheckedIn) {
        ...
        if (isEmployee) {
          // Employee checkout is a request — stay in checked-in state until HR approves
          setHasCompletedToday(true)
        } else {
          setIsCheckedIn(false)
          setHasCompletedToday(true)
  ...
  return (
    <div className="space-y-6">
        ...
        <div>
          <div className="flex items-center gap-2">
            <h1 className="text-xl sm:text-2xl font-extrabold text-[var(--color-text-heading)] flex items-center gap-2.5">
              ...
              <span>{isEmployee ? 'My Attendance' : 'Attendance Management'}</span>
            </h1>
            {isHRManager && (
          ...
          <p className="text-xs text-[var(--color-text-muted)] mt-1">
            {isEmployee
              ? 'Track your daily punch-in and punch-out. Requests are sent to HR for approval.'
      ...
      {/* ── EMPLOYEE-ONLY: Personal Punch View ── */}
      {isEmployee && (
        <div className="flex flex-col items-center justify-center gap-6 py-10">
          <div className="pp-card border border-[var(--color-border)] shadow-sm rounded-xl p-8 flex flex-col items-center gap-5 w-full max-w-sm text-center">
            <div
            ...
            <div>
              <p className="text-base font-extrabold text-[var(--color-text-heading)]">
                {hasCompletedToday
                  ? 'Punch Out Requested'
                  : isCheckedIn
              ...
              {isCheckedIn && !hasCompletedToday && (
                <p className="text-xs text-[var(--color-text-muted)] mt-1">
                  Punched in at <strong className="text-[var(--color-text-heading)]">{checkInTime}</strong>
                </p>
              )}
              {hasCompletedToday && (
                <p className="text-xs text-[var(--color-text-muted)] mt-1">
                  Your punch out request has been submitted and is pending HR approval.
                </p>
              )}
            </div>
            ...
            <button
              ...
              className={`w-full py-3 px-6 rounded-[6px] font-bold text-sm flex items-center justify-center gap-2 shadow-sm transition-colors cursor-pointer ${
                hasCompletedToday
            ...
            >
              ...
              <span>
                {hasCompletedToday
                  ...
                  : isCheckedIn
                  ? 'Punch Out (Send Request)'
                  : 'Punch In (Send Request)'}
              </span>
            </button>

            <p className="text-[11px] text-[var(--color-text-muted)] leading-relaxed">
              Your punch requests are sent to HR for approval. Check your dashboard for status updates.
            </p>
          </div>
        </div>
      )}

      {/* ── HR MANAGER / ADMIN: Full Attendance Console ── */}
      {!isEmployee && (
        <>
